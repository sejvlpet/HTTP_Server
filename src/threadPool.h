...
public:
    ...
    /**
     * Method to accept and add worker to the queue
     * @tparam T templated type to be stored in queue
     * @param worker object to be stored in queue
     */
template<class T>
    void Enqueue(T worker) {
        auto wrapper = std::make_shared<std::packaged_task<void()>>(std::move(worker));
        {
...
private:
    ...
    std::condition_variable _eventVar; //<! event var for thread sync
